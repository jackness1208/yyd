...
const task = {
  ...
  async stop() {
    ...
    if (extOs.IS_WINDOWS) {
      await extOs.runCMD('docker-machine restart');
    }
    return ids;
  },
  man(iEnv) {
    ...
    const manErrors = [
      ...
      ` ${chalk.gray('## cgroup error')}`,
      ` ${chalk.yellow.bold('docker-machine ssh default')}`,
      ` ${chalk.yellow.bold('sudo mkdir /sys/fs/cgroup/systemd')}`,
      ` ${chalk.yellow.bold('sudo mount -t cgroup -o none,name=systemd cgroup /sys/fs/cgroup/systemd')}`,
      ` ${chalk.gray('## --volume not work in windows os')}`,
      ` ${chalk.yellow('use docker terminal and change c:// => /c/')}`,
      ` ${chalk.gray('## --volume /d/path/to/project not exists')}`,
      ` ${chalk.gray('### case docker toolbox')}`,
      ` ${chalk.yellow('1.stop docker-machine =>')} ${chalk.yellow.bold('docker-machine stop')}`,
      '',
      ` ${chalk.yellow('2.In VirtualBox, add a Shared Folder: Settings > Shared Folders > Add share - this will be the directory where you want to locate your project, such as D:. Give it an appropriate Folder Name, such as d')}`,
      '',
      ` ${chalk.yellow('3.restart docker-machine =>')} ${chalk.yellow.bold('docker-machine start')}`
    ];
  ...
  async run() {
    ...
    if (config.volume) {
      Object.keys(config.volume).forEach((key) => {
        const rootPath = util.path.resolve(key)
          .replace(/^(\w):\//i, (str, $1) => {
    ...
    if (config.portMap) {
      ...
      if (extOs.IS_LINUX) {
        extV = '-v /sys/fs/cgroup:/sys/fs/cgroup:ro';
      }
