...
const task = {
  ...
  async start() {
    const config = fn.parseConfig();
    if (!config.tag) {
      throw 'config.tag must have value';
    }
    const cmd = `docker run -i -t ${config.tag}:${pkg.version} /bin/bash`;
  ...
  async build() {
    ...
    let cmd = `docker image build ./ -t ${config.tag}:${pkg.version}`;

    if (config.registryMirror) {
      cmd = `${cmd} --registry-mirror=https://${config.registryMirror}`;
    }
