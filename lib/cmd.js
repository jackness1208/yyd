...
const fn = {
  ...
  updateDockfile() {
    if (!fs.existsSync(DOCKER_PATH)) {
      ...
      if (!config.repsitory || !config.tag) {
        print.log.warn('update Dockerfile fail, (config.repsitory && config.tag) === false');
      } else {
...
const history = {
  ...
  add(tag) {
    ...
    self.source[tag] = {
      ...
      date: now,
      tag: tag,
      content: [HISTORY_ITEM_TMPL.replace('{$tag}', tag)]
    };
    ...
    const r = HISTORY_TMPL.replace('{$list}', (() => {
      ...
      vers.forEach((ver) => {
        ...
        if (item) {
          ...
          arr.push(HISTORY_ITEM_TMPL.replace('{$tag}', item.tag));
        }
      });
      return arr.join('\r\n');
    })());
